...
void allocate_objective_functions_context(NSGA2Type *nsga2Params, individual *ind, int n_ctx){
    ...
    ctx->spike_amount_per_neurons_per_sample = (int **)calloc(n_ctx * n_samples, sizeof(int *));
    ...
    if(nsga2Params->obj_functions_info->distance_matrix_req == 1){
        ctx->distance_matrix = (double *)calloc(n_ctx * n_samples * n_samples, sizeof(double));
    }
    ...
    ctx->centroid_info = (centroid_info_t *)calloc(n_ctx * n_classes, sizeof(centroid_info_t));
    ...
    ctx->mean_distance_per_sample = (double *)calloc(n_ctx * n_samples, sizeof(double));
    ctx->max_distance_per_sample_index = (int *)calloc(n_ctx * n_samples, sizeof(int));
    ctx->max_distance_per_sample = (double *)calloc(n_ctx * n_samples, sizeof(double));

    // matrix to store the distances between the classes (distances between the centroids of each class) [n_classes x n_classes]
    ctx->inter_class_distance_matrix = (double *)calloc(n_ctx * n_classes * n_classes, sizeof(double));

    // allocate memory for list to store accuracies
    ctx->acc_per_repetition = (double *)calloc(n_ctx * n_repetitions, sizeof(double));
    ctx->acc_per_class_per_repetition = (double **)calloc(n_ctx * n_classes, sizeof(double *)); // [n_classes x n_repetitions]
    for(i = 0; i<n_classes; i++)
        ctx->acc_per_class_per_repetition[i] = (double *)calloc(n_ctx * n_repetitions, sizeof(double));

    ctx->acc_per_class = (double *)calloc(n_ctx * n_classes, sizeof(double));
    ctx->accuracy = 0;
    ctx->confusion_matrix = (int **)calloc(n_ctx * n_repetitions, sizeof(int *)); // one confusion matrix per repetition
